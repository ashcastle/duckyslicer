...
enum class SupportPaintState(val code: Int) {
    ENFORCE(1),
    BLOCK(2),
    ;

    companion object {
        fun fromCode(code: Int): SupportPaintState? = entries.firstOrNull { it.code == code }
    }
}

data class SupportPaint(
    val facets: Map<Int, SupportPaintState> = emptyMap(),
) {
    fun paint(facetIndex: Int, state: SupportPaintState?): SupportPaint {
        ...
        require(next.size <= MAX_PAINTED_FACETS) { "Support paint is too large" }
        return if (next == facets) this else SupportPaint(next.toSortedMap())
    }

    fun writeSidecar(output: File) {
        require(facets.size <= MAX_PAINTED_FACETS) { "Support paint is too large" }
        ...
        check(output.length() == HEADER_BYTES + facets.size.toLong() * ENTRY_BYTES) {
            "Support paint could not be stored"
        }
    }

    companion object {
        val MAGIC = byteArrayOf('D'.code.toByte(), 'S'.code.toByte(), 'P'.code.toByte(), '1'.code.toByte())
