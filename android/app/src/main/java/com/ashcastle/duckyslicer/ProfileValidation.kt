...
internal object ProfileValidation {
    fun printer(profile: PrinterProfile): Boolean =
        profile.id.isSafeLabel() &&
            ...
            profile.printerStructure in PRINTER_STRUCTURES &&
            profile.bestObjectPositionX in 0f..1f &&
            profile.bestObjectPositionY in 0f..1f &&
            listOf(
            ...
            profile.templateCustomGcode.toByteArray(Charsets.UTF_8).size <= MAX_GCODE_TEMPLATE_LENGTH &&
            profile.timeLapseGcode.toByteArray(Charsets.UTF_8).size <= MAX_GCODE_TEMPLATE_LENGTH &&
