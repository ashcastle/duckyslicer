...
class VulkanAccelerationInstrumentedTest {
    ...
    fun vulkanProbeRejectsSoftwareDevicesFromAutomaticAcceleration() {
        ...
        if (capabilities.optBoolean("driverProbePassed")) {
            assertTrue(
                "A logical-device probe requires a compute queue",
                capabilities.getBoolean("computeQueueAvailable"),
            )
        }
        if (capabilities.optBoolean("autoCandidate")) {
            assertTrue(
                "An automatic candidate must pass the logical-device probe",
                capabilities.getBoolean("driverProbePassed"),
            )
            assertFalse(
                "Software Vulkan must never become an automatic candidate",
                capabilities.getBoolean("softwareDevice"),
            )
        }
        if (capabilities.optBoolean("softwareDevice")) {
            ...
            if (capabilities.optBoolean("driverProbePassed")) {
                assertTrue(
                    "A successfully probed software device must report its rejection reason",
