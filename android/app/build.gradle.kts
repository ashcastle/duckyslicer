...
val generatedProfileCatalog = generatedProfileAssets.map { it.file("profile_catalog_v101.bin") }
...
val buildSlicerRuntime = tasks.register<Exec>("buildSlicerRuntime") {
    ...
    inputs.file(repositoryRoot.resolve("native/slicer-runtime/project-3mf.patch"))
    inputs.file(repositoryRoot.resolve("native/slicer-runtime/profile-options.patch"))
    inputs.file(repositoryRoot.resolve("native/slicer-runtime/machine-motion-options.patch"))
    ...
    inputs.file(repositoryRoot.resolve("native/slicer-runtime/layer-pauses.patch"))
    ...
    inputs.file(repositoryRoot.resolve("native/slicer-runtime/process-notes.patch"))
...
val generateOrcaProfileCatalog = tasks.register<Exec>("generateOrcaProfileCatalog") {
    ...
    inputs.property("profileSchemaVersion", 118)
