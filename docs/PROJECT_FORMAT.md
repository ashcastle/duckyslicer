...
schema version `70`, and stores the selected plate plus a bounded `plates` list. Each plate
...
width are also retained, so portable projects preserve tool-change geometry, cooling output,
adhesion geometry, and the user's bed layout.

Schema 1 through 70 projects remain readable and migrate deterministically to one plate.
