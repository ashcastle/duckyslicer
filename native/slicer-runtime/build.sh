...
prepare_runtime_source() {
    ...
    if git -C "$SOURCE_ROOT" apply --reverse --check "$SCRIPT_DIR/printer-structure.patch" 2>/dev/null; then
        git -C "$SOURCE_ROOT" apply --reverse "$SCRIPT_DIR/printer-structure.patch"
    fi
    if git -C "$SOURCE_ROOT" apply --reverse --check "$SCRIPT_DIR/ironing-fan-speed.patch" 2>/dev/null; then
        git -C "$SOURCE_ROOT" apply --reverse "$SCRIPT_DIR/ironing-fan-speed.patch"
    fi
    if git -C "$SOURCE_ROOT" apply --reverse --check "$SCRIPT_DIR/gcode-line-numbers.patch" 2>/dev/null; then
        git -C "$SOURCE_ROOT" apply --reverse "$SCRIPT_DIR/gcode-line-numbers.patch"
    fi
    ...
    if git -C "$SOURCE_ROOT" apply --reverse --check "$SCRIPT_DIR/layer-pauses.patch" 2>/dev/null; then
        git -C "$SOURCE_ROOT" apply --reverse "$SCRIPT_DIR/layer-pauses.patch"
    fi
    ...
    if git -C "$SOURCE_ROOT" apply --reverse --check "$SCRIPT_DIR/nozzle-hardness-safety.patch" 2>/dev/null; then
        git -C "$SOURCE_ROOT" apply --reverse "$SCRIPT_DIR/nozzle-hardness-safety.patch"
    fi
    if git -C "$SOURCE_ROOT" apply --reverse --check "$SCRIPT_DIR/adaptive-pressure-advance.patch" 2>/dev/null; then
        git -C "$SOURCE_ROOT" apply --reverse "$SCRIPT_DIR/adaptive-pressure-advance.patch"
    fi
    ...
    git -C "$SOURCE_ROOT" apply --check "$SCRIPT_DIR/adaptive-pressure-advance.patch" 2>/dev/null || \
        die "runtime adaptive pressure-advance bridge contains unreviewed changes"
    git -C "$SOURCE_ROOT" apply "$SCRIPT_DIR/adaptive-pressure-advance.patch"
    git -C "$SOURCE_ROOT" apply --check "$SCRIPT_DIR/nozzle-hardness-safety.patch" 2>/dev/null || \
        die "runtime nozzle-hardness safety bridge contains unreviewed changes"
    git -C "$SOURCE_ROOT" apply "$SCRIPT_DIR/nozzle-hardness-safety.patch"
    ...
    git -C "$SOURCE_ROOT" apply --check "$SCRIPT_DIR/layer-pauses.patch" 2>/dev/null || \
        die "runtime layer-pause bridge contains unreviewed changes"
    git -C "$SOURCE_ROOT" apply "$SCRIPT_DIR/layer-pauses.patch"
    ...
    git -C "$SOURCE_ROOT" apply --check "$SCRIPT_DIR/gcode-line-numbers.patch" 2>/dev/null || \
        die "runtime G-code line-number bridge contains unreviewed changes"
    git -C "$SOURCE_ROOT" apply "$SCRIPT_DIR/gcode-line-numbers.patch"
    git -C "$SOURCE_ROOT" apply --check "$SCRIPT_DIR/ironing-fan-speed.patch" 2>/dev/null || \
        die "runtime ironing-fan bridge contains unreviewed changes"
    git -C "$SOURCE_ROOT" apply "$SCRIPT_DIR/ironing-fan-speed.patch"
    git -C "$SOURCE_ROOT" apply --check "$SCRIPT_DIR/printer-structure.patch" 2>/dev/null || \
        die "runtime printer-structure bridge contains unreviewed changes"
    git -C "$SOURCE_ROOT" apply "$SCRIPT_DIR/printer-structure.patch"
