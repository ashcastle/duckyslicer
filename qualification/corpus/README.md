...
Android output with the pinned slicing engine. The fixtures are generated by
...
Then run `python3 tools/run_desktop_orca_qualification.py`; it rebuilds the CLI when the pinned
source, compatibility inputs, toolchain, or binary digest changes, then compares every case.
...
release gate. Release preparation additionally requires the complete, source-bound desktop-engine
comparison report; a physical threshold report by itself is insufficient.
